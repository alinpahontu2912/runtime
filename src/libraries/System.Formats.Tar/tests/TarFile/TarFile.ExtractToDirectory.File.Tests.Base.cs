...
{
    public abstract class TarFile_ExtractToDirectory_File_Tests_Base : TarTestsBase
    {
        protected abstract Task ExtractToDirectoryAsync(string sourceFileName, string destinationDirectoryName, bool overwriteFiles);
        protected abstract Task ExtractToDirectoryAsync(Stream source, string destinationDirectoryName, bool overwriteFiles);
        protected abstract Task ExtractToDirectoryAsync(string sourceFileName, string destinationDirectoryName, TarExtractOptions options);

        [Fact]
        public async Task InvalidPaths_Throw()
        {
            await Assert.ThrowsAsync<ArgumentNullException>(() => ExtractToDirectoryAsync(sourceFileName: null, destinationDirectoryName: "path", overwriteFiles: false));
            await Assert.ThrowsAsync<ArgumentException>(() => ExtractToDirectoryAsync(sourceFileName: string.Empty, destinationDirectoryName: "path", overwriteFiles: false));
            await Assert.ThrowsAsync<ArgumentNullException>(() => ExtractToDirectoryAsync(sourceFileName: "path", destinationDirectoryName: null, overwriteFiles: false));
            await Assert.ThrowsAsync<ArgumentException>(() => ExtractToDirectoryAsync(sourceFileName: "path", destinationDirectoryName: string.Empty, overwriteFiles: false));
        }

        [Fact]
        public async Task NonExistentFile_Throws()
        {
            ...
            await Assert.ThrowsAsync<FileNotFoundException>(() => ExtractToDirectoryAsync(sourceFileName: filePath, destinationDirectoryName: dirPath, overwriteFiles: false));
        }
        ...
        public async Task NonExistentDirectory_Throws()
        {
            ...
            await Assert.ThrowsAsync<DirectoryNotFoundException>(() => ExtractToDirectoryAsync(sourceFileName: filePath, destinationDirectoryName: dirPath, overwriteFiles: false));
        }
        ...
        public async Task SetsLastModifiedTimeOnExtractedFiles()
        {
            ...
            await ExtractToDirectoryAsync(sourceFileName: tarFile, destinationDirectoryName: outDir, overwriteFiles: false);
        ...
        public async Task SetsLastModifiedTimeOnExtractedDirectories()
        {
            ...
            await ExtractToDirectoryAsync(sourceFileName: tarFile, destinationDirectoryName: toDir, overwriteFiles: false);
        ...
        public async Task Extract_Archive_File(TestTarFormat testFormat)
        {
            ...
            await ExtractToDirectoryAsync(sourceArchiveFileName, destination.Path, overwriteFiles: false);

            Assert.True(File.Exists(filePath));
        }

        [Fact]
        public async Task Extract_Archive_File_OverwriteTrue()
        {
            ...
            await ExtractToDirectoryAsync(archivePath, destination.Path, overwriteFiles: true);
        ...
        [Fact]
        public async Task Extract_Archive_File_OverwriteFalse()
        {
            ...
            await Assert.ThrowsAsync<IOException>(() => ExtractToDirectoryAsync(sourceArchiveFileName, destination.Path, overwriteFiles: false));
        }

        [Fact]
        public async Task Extract_AllSegmentsOfPath()
        {
            ...
            await ExtractToDirectoryAsync(archivePath, destination.Path, overwriteFiles: false);
        ...
        [Fact]
        public async Task ExtractArchiveWithEntriesThatStartWithSlashDotPrefix()
        {
            using TempDirectory root = new TempDirectory();
        ...
        public async Task UnixFileModes(bool overwrite)
        {
            ...
            await ExtractToDirectoryAsync(archivePath, destination.Path, overwriteFiles: overwrite);
        ...
        public async Task LinkBeforeTarget()
        {
            using TempDirectory source = new TempDirectory();
            using TempDirectory destination = new TempDirectory();
        ...
        public async Task HardLinkExtractionRoundtrip(TarEntryFormat format, TarHardLinkMode linkMode)
        {
            using TempDirectory root = new TempDirectory();
            ...
            await ExtractToDirectoryAsync(archivePath, destination, overwriteFiles: false);
        ...
        public async Task HardLinkExtraction_CopyContents()
        {
            using TempDirectory root = new TempDirectory();
            ...
            await ExtractToDirectoryAsync(archivePath, destination, extractOptions);
