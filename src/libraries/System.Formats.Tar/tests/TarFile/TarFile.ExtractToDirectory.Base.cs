...
public abstract class TarFile_ExtractToDirectory_Tests : TarTestsBase
{
    protected abstract Task ExtractToDirectoryAsync(Stream source, string destinationDirectoryName, bool overwriteFiles);
        ...
        public async Task NullStream_Throws() =>
            await Assert.ThrowsAsync<ArgumentNullException>(() => ExtractToDirectoryAsync(source: null, destinationDirectoryName: "path", overwriteFiles: false));
        ...
        public async Task InvalidPath_Throws()
        {
            ...
            {
                await Assert.ThrowsAsync<ArgumentNullException>(() => ExtractToDirectoryAsync(archive, destinationDirectoryName: null, overwriteFiles: false));
                await Assert.ThrowsAsync<ArgumentException>(() => ExtractToDirectoryAsync(archive, destinationDirectoryName: string.Empty, overwriteFiles: false));
            }
        }

        [Fact]
        public async Task UnreadableStream_Throws()
        {
            using (MemoryStream archive = new MemoryStream())
            {
                using (WrappedStream unreadable = new WrappedStream(archive, canRead: false, canWrite: true, canSeek: true))
                {
                    await Assert.ThrowsAsync<ArgumentException>(() => ExtractToDirectoryAsync(unreadable, destinationDirectoryName: "path", overwriteFiles: false));
                }
            }
        }
        ...
        public async Task NonExistentDirectory_Throws()
        {
            using (TempDirectory root = new TempDirectory())
            {
                ...
                using (MemoryStream archive = new MemoryStream())
                {
                    await Assert.ThrowsAsync<DirectoryNotFoundException>(() => ExtractToDirectoryAsync(archive, destinationDirectoryName: dirPath, overwriteFiles: false));
                }
            }
        }

        [Fact]
        public async Task ExtractEntry_ManySubfolderSegments_NoPrecedingDirectoryEntries()
        {
            using (TempDirectory root = new TempDirectory())
            {
                ...
                await using (MemoryStream archive = new MemoryStream())
                {
                    ...
                    await ExtractToDirectoryAsync(archive, root.Path, overwriteFiles: false);
        ...
        [Fact]
        public async Task ExtractEntry_DockerImageTarWithFileTypeInDirectoriesInMode_SuccessfullyExtracts()
        {
            using (TempDirectory root = new TempDirectory())
            {
                await using MemoryStream archiveStream = GetTarMemoryStream(CompressionMethod.Uncompressed, "misc", "docker-hello-world");
                await ExtractToDirectoryAsync(archiveStream, root.Path, overwriteFiles: true);
        ...
        public async Task ExtractEntry_PodmanImageTarWithRelativeSymlinksPointingInExtractDirectory_SuccessfullyExtracts()
        {
            using (TempDirectory root = new TempDirectory())
            {
        ...
        public async Task Extract_LinkEntry_TargetOutsideDirectory(TarEntryType entryType)
        {
            await using (MemoryStream archive = new MemoryStream())
            {
                ...
                using (TempDirectory root = new TempDirectory())
                {
                    await Assert.ThrowsAnyAsync<IOException>(() => ExtractToDirectoryAsync(archive, root.Path, overwriteFiles: false));
        ...
        public async Task Extract_SymbolicLinkEntry_TargetInsideDirectory(TarEntryFormat format) =>
            await Extract_LinkEntry_TargetInsideDirectory_Internal(TarEntryType.SymbolicLink, format, null);
        ...
        public async Task Extract_HardLinkEntry_TargetInsideDirectory(TarEntryFormat format) =>
            await Extract_LinkEntry_TargetInsideDirectory_Internal(TarEntryType.HardLink, format, null);

        [ConditionalTheory(typeof(MountHelper), nameof(MountHelper.CanCreateSymbolicLinks))]
        [InlineData(TarEntryFormat.Pax)]
        [InlineData(TarEntryFormat.Gnu)]
        public async Task Extract_SymbolicLinkEntry_TargetInsideDirectory_LongBaseDir(TarEntryFormat format) =>
            await Extract_LinkEntry_TargetInsideDirectory_Internal(TarEntryType.SymbolicLink, format, new string('a', 99));

        [ConditionalTheory(typeof(PlatformDetection), nameof(PlatformDetection.SupportsHardLinkCreation))]
        [InlineData(TarEntryFormat.Pax)]
        [InlineData(TarEntryFormat.Gnu)]
        public async Task Extract_HardLinkEntry_TargetInsideDirectory_LongBaseDir(TarEntryFormat format) =>
            await Extract_LinkEntry_TargetInsideDirectory_Internal(TarEntryType.HardLink, format, new string('a', 99));
        ...
        private async Task Extract_LinkEntry_TargetInsideDirectory_Internal(TarEntryType entryType, TarEntryFormat format, string subfolder)
        {
            using (TempDirectory root = new TempDirectory())
            {
                ...
                await using (MemoryStream archive = new MemoryStream())
                {
                    ...
                    await ExtractToDirectoryAsync(archive, baseDir, overwriteFiles: false);
        ...
        public async Task Extract_UnseekableStream_BlockAlignmentPadding_DoesNotAffectNextEntries(int contentSize)
        {
            ...
            await ExtractToDirectoryAsync(decompressor, destination.Path, overwriteFiles: true);

            Assert.Equal(2, Directory.GetFileSystemEntries(destination.Path, "*", SearchOption.AllDirectories).Count());
        }

        [Fact]
        public async Task PaxNameCollision_DedupInExtendedAttributes()
        {
            ...
            await ExtractToDirectoryAsync(stream, root.Path, overwriteFiles: true);
        ...
        public async Task UnseekableStreams_RoundTrip(TestTarFormat testFormat)
        {
            ...
            await ExtractToDirectoryAsync(sourceUnseekableArchiveStream, root.Path, overwriteFiles: false);
        ...
        {
            await ExtractToDirectory_ExactRootDirMatch_RegularFile_And_Directory_Throws_Internal(format, entryType, fileName, inverted: false);
            await ExtractToDirectory_ExactRootDirMatch_RegularFile_And_Directory_Throws_Internal(format, entryType, fileName, inverted: true);
        }

        [Fact]
        [SkipOnPlatform(TestPlatforms.iOS | TestPlatforms.tvOS, "The temporary directory on Apple mobile platforms exceeds the path length limit.")]
        public async Task ExtractToDirectory_ExactRootDirMatch_Directory_Relative_Throws()
        {
            ...
            await ExtractRootDirMatch_Verify_Throws(TarEntryFormat.Ustar, TarEntryType.Directory, destinationFolderPath, dirPath1, linkTargetPath: null);
            await ExtractRootDirMatch_Verify_Throws(TarEntryFormat.Ustar, TarEntryType.Directory, destinationFolderPath, dirPath2, linkTargetPath: null);
        }
        ...
        {
            await ExtractToDirectory_ExactRootDirMatch_Links_Throws(format, TarEntryType.HardLink, inverted: false);
            await ExtractToDirectory_ExactRootDirMatch_Links_Throws(format, TarEntryType.HardLink, inverted: true);
        }
        ...
        {
            await ExtractToDirectory_ExactRootDirMatch_Links_Throws(format, TarEntryType.SymbolicLink, inverted: false);
            await ExtractToDirectory_ExactRootDirMatch_Links_Throws(format, TarEntryType.SymbolicLink, inverted: true);
        }

        [ConditionalFact(typeof(MountHelper), nameof(MountHelper.CanCreateSymbolicLinks))]
        public async Task ExtractToDirectory_ExactRootDirMatch_SymLinks_TargetOutside_Throws()
        {
            string entryFolderName = "folder";
            string destinationFolderName = "folderSibling";
            ...
            await ExtractRootDirMatch_Verify_Throws(TarEntryFormat.Ustar, TarEntryType.Directory, destinationFolderPath, linkPath, linkTargetPath1);
            await ExtractRootDirMatch_Verify_Throws(TarEntryFormat.Ustar, TarEntryType.Directory, destinationFolderPath, linkPath, linkTargetPath2);
        }

        private async Task ExtractToDirectory_ExactRootDirMatch_RegularFile_And_Directory_Throws_Internal(TarEntryFormat format, TarEntryType entryType, string fileName, bool inverted)
        {
            // inverted == false:
            ...
            await ExtractRootDirMatch_Verify_Throws(format, entryType, destinationFolderPath, filePath, linkTargetPath: null);
        }

        private async Task ExtractToDirectory_ExactRootDirMatch_Links_Throws(TarEntryFormat format, TarEntryType entryType, bool inverted)
        {
            // inverted == false:
            //   destination: folderSibling/
            //   entry folder: folder/ (does not match destination)
            ...
            await ExtractRootDirMatch_Verify_Throws(format, entryType, destinationFolderPath, linkPath, linkTargetPath);
        }

        private async Task ExtractRootDirMatch_Verify_Throws(TarEntryFormat format, TarEntryType entryType, string destinationFolderPath, string entryFilePath, string linkTargetPath)
        {
            ...
            await Assert.ThrowsAsync<IOException>(() => ExtractToDirectoryAsync(archive, destinationFolderPath, overwriteFiles: false));
        ...
        public async Task PaxExtraction_EntryNameMatchesExtractedPath(string headerName, string eaName, string expectedApiName)
        {
            ...
            await using (var scanStream = new MemoryStream(archive))
            await using (var reader = new TarReader(scanStream))
            {
                TarEntry entry = await reader.GetNextEntryAsync();
                Assert.NotNull(entry);
            ...
            await using var extractStream = new MemoryStream(archive);
            await ExtractToDirectoryAsync(extractStream, root.Path, overwriteFiles: false);
        ...
        [Fact]
        public async Task PaxExtraction_PathTraversalInEA_IsBlocked()
        {
            byte[] content = "test data"u8.ToArray();
            ...
            await Assert.ThrowsAsync<IOException>(() =>
                ExtractToDirectoryAsync(extractStream, root.Path, overwriteFiles: false));
        }

        [Theory]
        [InlineData(10, 50)]  // EA larger than header
        [InlineData(100, 25)] // EA smaller than header
        public async Task PaxExtraction_EntryLengthMatchesExtractedFileSize(int dataSize, long eaSize)
        {
            ...
            await using (var reader = new TarReader(scanStream))
            {
                TarEntry entry = await reader.GetNextEntryAsync(copyData: true);
                Assert.NotNull(entry);
        ...
        [Fact]
        public async Task PaxExtraction_SizeAmplification_EntryLengthMatchesExtraction()
        {
            int entryCount = 10;
            ...
            {
                TarEntry e;
                while ((e = await reader.GetNextEntryAsync(copyData: true)) is not null)
                {
